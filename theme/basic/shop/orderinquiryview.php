...
<div id="sod_fin">
    ...
    <div class="sod_left">
        ...
        if($od['od_settle_case'] == '정기(구독)' || $od['od_settle_case'] == '신용카드' || $od['od_settle_case'] == '수기결제' || $od['od_settle_case'] == 'KAKAOPAY' || is_inicis_order_pay($od['od_settle_case']) ) {
        // e: cookiepay-plugin
            $app_no_subj = '승인번호';
            $app_no = $od['od_app_no'];
            $disp_bank = false;
            ...
            <ul>
	            ...
	            <li>
	                ...
	                <span>
	                    ...
	                    if($od['od_settle_case'] == '정기(구독)' || $od['od_settle_case'] == '신용카드' || $od['od_settle_case'] == '수기결제' || is_inicis_order_pay($od['od_settle_case']) || (shop_is_taxsave($od, true) && $misu_price == 0) )
	                    {
                        // e: cookiepay-plugin
	                        if($od['od_pg'] == 'lg') {
	                            require_once G5_SHOP_PATH.'/settle_lg.inc.php';
                                ...
                                if (array_key_exists($default['de_pg_service'], COOKIEPAY_PG)) {
                                    $card_receipt_script = "cookiepayReceipt('".$od['od_tno']."');";
                                }
...
// s: cookiepay-plugin - 쿠키페이 처리
function cookiepayBase64Encode(str) {
    return btoa(encodeURIComponent(str).replace(/%([0-9A-F]{2})/g, function(match, p1) {
        return String.fromCharCode('0x' + p1);
    }));
}

function cookiepayReceipt(tid) {
    var tid = cookiepayBase64Encode(tid);
    window.open(
