...
foreach ($check_sanitize_keys as $column) {
    if ($column == 'cookiepay_de_pg_service') {
        if (mb_strpos($de_pg_service, 'COOKIEPAY') !== false && isset($cookiepay_de_pg_service) && !empty($cookiepay_de_pg_service)) {
            $de_pg_service = $cookiepay_de_pg_service;
        }
    } else {
        array_push($cookiepayUpdateSet, "{$column}='{$$column}'");
    }
}
