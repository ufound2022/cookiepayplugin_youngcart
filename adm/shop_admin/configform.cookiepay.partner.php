...
<?php // s: cookiepay-plugin ?>
<style>
.cookiepay-only {display: none;}
</style>
<?php // e: cookiepay-plugin ?>
...
<section id ="anc_scf_payment">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr class="cookiepay-only">
            ...
            <td>
                <input type="hidden" name="de_pg_service" id="de_pg_service" value="COOKIEPAY_TS" >
                <?php echo help('쇼핑몰에서 사용할 결제대행사를 선택합니다.'); ?>
                <ul class="de_pg_tab">

                <?php // s: cookiepay-plugin ?>
                    <li class="tab-current"><a href="#cookiepay_info_anchor" data-value="COOKIEPAY_AL" title="쿠키페이 선택하기">쿠키페이</a></li>
                <?php // e: cookiepay-plugin ?>
        ...
        <?php // s: cookiepay-plugin ?>
        <tr id="cookiepay_info_anchor">
            <th scope="row">
                <label for="cookiepay_de_pg_service">결제대행사</label><br>
            ...
            <td>
                ...
                $i = 0;
                foreach (COOKIEPAY_PG as $cookiepayPgCode => $cookiepayPgName) {
                    $checked = "";
                    $active = "";
                
                    if ($default['de_pg_service'] == $cookiepayPgCode) {
                        $checked = "checked";
                        $active = "tab-current";
                    }
                    
                    echo '<label class="cookiepay-only" style="margin-right:15px;"><input type="radio" name="cookiepay_de_pg_service" id="cookiepay_de_pg_service_'.$i.'" value="'.$cookiepayPgCode.'" '.$checked.'> '.$cookiepayPgName.'</label>';

                    echo '<li class="'.$active.'"><a href="#" class="choice-pg-tab" data-radioid="cookiepay_de_pg_service_'.$i.'" data-value="'.$cookiepayPgCode.'" title="'.$cookiepayPgName.' 선택하기" >'.$cookiepayPgName.'</a></li>';
        ...
        $i = 0;
        foreach (COOKIEPAY_PG as $cookiepayPgCode => $cookiepayPgName) { 
            $cookiepayPgCodeLower = strtolower($cookiepayPgCode);
        ?>
        ...
        <tr class="pg_info_fld kcp_info_fld cookiepay-only" id="kcp_info_anchor">
            <th scope="row">
        ...
        <tr class="pg_info_fld kcp_info_fld cookiepay-only">
        ...
        <tr class="pg_info_fld lg_info_fld cookiepay-only" id="lg_info_anchor">
            <th scope="row">
        ...
        <tr class="pg_info_fld lg_info_fld cookiepay-only">
        ...
        <tr class="pg_info_fld inicis_info_fld cookiepay-only" id="inicis_info_anchor">
            <th scope="row">
        ...
        <tr class="pg_info_fld inicis_info_fld cookiepay-only">
        ...
        <tr class="kakao_info_fld cookiepay-only">
            <th scope="row">
        ...
        <tr class="kakao_info_fld cookiepay-only" style="display:none">
        ...
        <tr class="naver_info_fld cookiepay-only">
            <th scope="row">
...
function fconfig_check(f)
{
    ...
    // s: cookiepay-plugin
    if (f.de_pg_service.value.indexOf('COOKIEPAY') != -1) {
        var cookiepayUsePgValue = $('input[name="cookiepay_de_pg_service"]:checked').val();
        if (!cookiepayUsePgValue) {
...
// s: cookiepay-plugin
function cookiepayIdKeySection(){
    var usePgValue = $('input[name="cookiepay_de_pg_service"]:checked').val();
    if (!usePgValue) {
        $(".cookiepay-pglist").css("display", "none");
    } else {
...
function cookiepayChoicePG(){
    $('input[name="cookiepay_de_pg_service"]').on("change", function(e){
        var usePgValue = $('input[name="cookiepay_de_pg_service"]:checked').val();
        var usePg = usePgValue.toLowerCase();
        $(".cookiepay-pglist").css("display", "none");
        $(`.${usePg}_info_fld`).css("display", "");
...
function choicePgTab(){
    $('.choice-pg-tab').on("click", function(e){
        var radioId = $(this).data("radioid");
        $("#"+radioId).prop("checked", true);
        
        var usePgValue = $(this).data("value");
        var usePg = usePgValue.toLowerCase();
        $(".cookiepay-pglist").css("display", "none");
        $(`.${usePg}_info_fld`).css("display", "");
    });
