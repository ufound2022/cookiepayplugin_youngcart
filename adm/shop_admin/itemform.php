    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <td>
                ...
                ## 영카트 플러그인 > 정기(구독) > S
                -->
                <input type="checkbox" name="it_type6" id="it_type6" value="1" <?php echo ($it['it_type6'] ? "checked" : ""); ?> id="it_type6" onClick="last_pay_cnt_area_check()"> <!-- // 정기(구동)추가 -->
                <label for="it_type6">정기(구독) <img src="<?php echo G5_SHOP_URL; ?>/img/icon_subscription.png" alt=""></label>
                <!-- 
                ## 영카트 플러그인 > 정기(구독) > E
                -->                
            </td>
        ...
        ## 영카트 플러그인 > 정기(구독) > S
        -->
        <input type="hidden" name="last_pay_cnt_check" id="last_pay_cnt_check" value="<?=$it['it_type6']?>">
        <tr <?php if($it['it_type6'] != "1") { echo "style='display:none' "; } ?> id="last_pay_cnt_area">
            <th scope="row"><label for="last_pay_cnt">정기(구독) 납부횟수</label></th>
            <td>
                <?php echo help("현재 상품의 정기(구독) 납부횟수를 지정합니다.\n<b>입력하지 않으면 기본값은 0입니다. (약정횟수 제한없음 > 해지전까지 계속 반복결제 됩니다.)</b>"); ?>
                <input type="text" name="last_pay_cnt" id="last_pay_cnt" value="<?php echo $it['last_pay_cnt']; ?>" id="last_pay_cnt" class="frm_input" size="6">
            </td>
            <td class="td_grpset">
                <input type="checkbox" name="chk_ca_last_pay_cnt" value="1" id="chk_ca_last_pay_cnt">
                <label for="chk_ca_it_order">분류적용</label>
                <input type="checkbox" name="chk_all_last_pay_cnt" value="1" id="chk_all_last_pay_cnt">
                <label for="chk_all_it_order">전체적용</label>
            </td>
        </tr>
        <!-- 
...
function last_pay_cnt_area_check() { 

    if($('input[name="it_type6"]').is(":checked") == true) {
        $("#last_pay_cnt_check").val('1');
        $("#last_pay_cnt_area").show();
    } else {
        $("#last_pay_cnt_check").val('');
        $("#last_pay_cnt_area").hide();
    }
}
</script>
