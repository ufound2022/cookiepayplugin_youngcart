...
<input type="hidden" name="subscription"> <!-- 쿠키페이 정기결제 -->
    ...
    <div class="sl_option">
        ...
        <div id="sit_ov_btn">
            <?php if ($is_orderable) { ?>
                ...
                <?php
				## 영카트 플러그인 > 정기(구독) > S 
				if(!empty($default['de_cookiepay_subscription_use']) && !empty($it['it_type6'])) { 
				## 영카트 플러그인 > 정기(구독) > E
				?>
				...
				if(empty($is_member)) { 
				?>sit_btn_buy
                <input type="button" onclick="alert('회원가입후 이용하여 주십시요');" value="정기(구독)" id="sit_btn_buy" class="btn_submit">
                <? } else { ?>
                <input type="submit" onclick="document.pressed=this.value;" value="정기(구독)" id="sit_btn_buy" class="btn_submit">
                <? } ?>
...
function fitem_submit(f)
{
    ...
    if (document.pressed == "바로구매") { // 바로구매
        f.sw_direct.value = 1;
		f.subscription.value = "";
    } else { // 정기(구독)
        f.sw_direct.value = 1;
		f.subscription.value = 1;
    }
